				 ...
				 unsigned int type, unsigned int code, int value)
{
	if(my_input_handle_event == NULL) {
		my_input_handle_event = (void (*)(struct input_dev *, unsigned int, unsigned int, int))zoe_kallsyms_lookup_name("input_handle_event");
	}

	if (!my_input_handle_event) {
		pr_err("[zoe] Holy fuck!Failed to find input_handle_event\n");
		return -1;
	}
...
struct input_dev* find_touch_device(void) {
	...
	input_dev_list = (struct list_head *)zoe_kallsyms_lookup_name("input_dev_list");
	input_mutex = (struct mutex *)zoe_kallsyms_lookup_name("input_mutex");
	...
	list_for_each_entry(dev, input_dev_list, node) {
		if (test_bit(EV_ABS, dev->evbit) &&
            ...
            pr_info("[zoe] Name: %s, Bus: %d Vendor: %d Product: %d Version: %d\n",
					dev->name,
...
int input_event_cache(unsigned int type, unsigned int code, int value, int lock) {
	if (!my_input_handle_event) {
		pr_err("[zoe] Failed to find input_handle_event\n");
		return -EINVAL;
	}
	...
	struct zoe_touch_event* event = &pool->events[pool->size++];
	event->type = type;
	event->code = code;
	event->value = value;
	if (lock)
...
static void handle_cache_events(struct input_dev* dev) {
	...
	for (int i = 0; i < pool->size; ++i) {
		struct zoe_touch_event event = pool->events[i];
...
//	if (type == EV_ABS) {
//		pr_info("[zoe] input_event(%u, %u, %d)", type, code, value);
//	}
...
int init_input_dev(void) {
	...
	if (!my_input_handle_event) {
		pr_err("[zoe] failed to find my_input_handle_event\n");
		return -1;
	}
