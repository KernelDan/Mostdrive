...
static int zoe_release(struct socket *sock) {
	...
	struct zoe_sock *os = (struct zoe_sock *) ((char *) sock->sk + sizeof(struct sock));
	...
	//pr_info("[zoe] zoe socket released!\n");
	return 0;
}

static __poll_t zoe_poll(struct file *file, struct socket *sock,
						 struct poll_table_struct *wait) {
	return 0;
}

static int zoe_setsockopt(struct socket *sock, int level, int optname,
...
__always_inline int zoe_get_process_pid(int len, char __user *process_name_user) {
	int err;
	pid_t pid;
	char* process_name;
...
__always_inline int zoe_get_process_module_base(int len, pid_t pid, char __user *module_name_user, int flag) {
	int err;
	char* module_name;
 ...
 */
static int zoe_getsockopt(struct socket *sock, int level, int optname,
						  char __user *optval, int __user *optlen)
{
	...
	if (!sk)
		return -EINVAL;
	os = ((struct zoe_sock*)((char *) sock->sk + sizeof(struct sock)));

	pr_debug("[zoe] getsockopt: %d\n", optname);
	switch (optname) {
		case REQ_GET_PROCESS_PID: {
			ret = zoe_get_process_pid(level, optval);
			if (ret) {
				pr_err("[zoe] zoe_get_process_pid failed: %d\n", ret);
			}
			break;
		}
		...
		case REQ_ATTACH_PROCESS: {
			...
			os->pid = level;
			pr_info("[zoe] attached process: %d\n", level);
			ret = 0;
			break;
		}
...
int zoe_mmap(struct file *file, struct socket *sock,
	...
	if (!ret) {
		pr_err("[zoe] remap_process_memory failed: %d\n", ret);
	}
	return ret;
}

int zoe_ioctl(struct socket * sock, unsigned int cmd, unsigned long arg) {
	...
	if (cmd == CMD_TOUCH_CLICK_DOWN) {
		...
		if (pool->size >= MAX_EVENTS) {
			pr_warn("[zoe] event pool is full!\n");
			pool->size = 0;
		}
		...
		if (copy_to_user(event_user, &event, sizeof(struct touch_event_base))) {
			pr_err("[zoe] copy_to_user failed: %s\n", __func__);
			return -EACCES;
		}
	...
	if (cmd == CMD_COPY_PROCESS) {
		...
		const struct zoe_sock *os = (struct zoe_sock *) ((char *) sock->sk + sizeof(struct sock));
		if (os->pid == 0) {
			return -ESRCH;
		}
		...
		if (my_copy_process == NULL) {
			my_copy_process = (void*) zoe_kallsyms_lookup_name("copy_process");
		}

		if (my_copy_process == NULL) {
			pr_err("[zoe] copy_process not found!\n");
			return -EFAULT;
		}
		...
		if (!new_task) {
			pr_err("[zoe] copy_process failed!\n");
			return -EFAULT;
		}
	...
	if (cmd == CMD_PROCESS_MALLOC) {
		...
		if (os->cached_count >= MAX_CACHE_KERNEL_ADDRESS_COUNT) {
			pr_err("[zoe] cached_addr_array is full!\n");
			return -ENOMEM;
		}
		...
		struct pid *pid_struct = find_get_pid(os->pid);
		if (!pid_struct) {
			pr_err("[zoe] failed to find pid_struct: %s\n", __func__);
			return -ESRCH;
		}
		...
		put_pid(pid_struct);
		if(!task) {
			pr_err("[zoe] failed to get task from pid_struct: %s\n", __func__);
			return -ESRCH;
		}
		...
		if (!mm) {
			pr_err("[zoe] failed to get mm from task: %s\n", __func__);
			return -ESRCH;
		}
		...
		if (addr == 0) {
			...
			mmput(mm);
			atomic_set(&os->remap_in_progress, 0);
			pr_err("[zoe] get_unmapped_area_mm failed: %s\n", __func__);
			return -ENOMEM;
		}
		...
		if (alloc_process_special_memory_mm(mm, addr, PAGE_SIZE, writable)) {
			...
			mmput(mm);
			atomic_set(&os->remap_in_progress, 0);
			pr_err("[zoe] alloc_process_special_memory_mm failed: %s\n", __func__);
			return -ENOMEM;
		}
		...
		pr_info("[zoe] malloced kernel address: 0x%lx, pfn: 0x%lx, magic: 0x%lx\n", kaddr, pfn, *(unsigned long*) kaddr);
		return -2033;
	}
	...
	if (cmd == CMD_HIDE_VMA) {
		...
		if (copy_from_user(&args, (struct hide_vma_args __user*) arg, sizeof(struct hide_vma_args))) {
			pr_err("[zoe] copy_from_user failed: %s\n", __func__);
			return -EACCES;
		}
		...
		if (args.mode == HIDE_X) {
			...
			pr_warn("[zoe] hide mode not supported!\n");
			return -ENOSYS;
		}
...
int zoe_sendmsg(struct socket *sock, struct msghdr *m,
                size_t total_len
) {
	return 0;
}

static struct proto zoe_proto = {
	.name = "NFC_LLCP",
	.owner = THIS_MODULE,
	.obj_size = sizeof(struct sock) + sizeof(struct zoe_sock),
};

static struct proto_ops zoe_proto_ops = {
	.family = PF_DECnet,
	.owner = THIS_MODULE,
	.release = zoe_release,
	.bind = sock_no_bind,
	...
	.setsockopt	= zoe_setsockopt,
	.getsockopt	= zoe_getsockopt,
	.sendmsg	= zoe_sendmsg,
...
static int zoe_create(struct net *net, struct socket *sock, int protocol,
					  int kern)
{
	uid_t caller_uid;
	struct sock *sk;
	...
	if (caller_uid != 0) {
		pr_warn("[zoe] Only root can create zoe socket!\n");
		return -EAFNOSUPPORT;
	}

	if (sock->type != SOCK_RAW) {
		//pr_warn("[zoe] a zoe socker must be SOCK_RAW!\n");
		return -ENOKEY;
	}

	sock->state = SS_UNCONNECTED;

	sk = sk_alloc(net, PF_INET, GFP_KERNEL, &zoe_proto, kern);
	if (!sk) {
		pr_warn("[zoe] sk_alloc failed!\n");
		return -ENOBUFS;
	}

	os = (struct zoe_sock*)((char *) sk + sizeof(struct sock));

	zoe_proto_ops.family = free_family;
	sock->ops = &zoe_proto_ops;
	sock_init_data(sock, sk);

	// Initialize the zoe_sock
	os->pid = 0;
	os->pfn = 0;
	atomic_set(&os->remap_in_progress, 0);
	os->cached_count = 0;
...
static struct net_proto_family zoe_family_ops = {
	.family = PF_DECnet,
	.create = zoe_create,
	.owner	= THIS_MODULE,
};
...
static int register_free_family(void) {
	...
	for(family = free_family; family < NPROTO; family++) {
		zoe_family_ops.family = family;
		err = sock_register(&zoe_family_ops);
		if (err)
			continue;
		else {
			free_family = family;
			pr_info("[zoe] Find free proto_family: %d\n", free_family);
			return 0;
		}
	}

	pr_err("[zoe] Can't find any free proto_family!\n");
	return err;
}
...
int init_server(void) {
	...
	err = proto_register(&zoe_proto, 1);
	if (err)
		goto out;
